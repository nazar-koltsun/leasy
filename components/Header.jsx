...
const Header = ({ isDashboard = false, isAuth = false, className }) => {
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex gap-3 items-center order-3 max-700:mt-4 max-620:gap-3", isAuth && 'gap-5')}>
        ...
        <Button
          variant="flat"
          className={cn("group transition duration-300 ease-in-out", flatBtnBaseClasses, !isAuth && 'mr-2')}
        >
        ...
        {isAuth && (
          <>
            <Button variant='flat' className={cn('group relative', flatBtnBaseClasses)}>
            ...
            <Button className='group relative w-12 h-12 p-0 rounded-full bg-transparent border border-[#ddd] 
            hover:bg-transparent shadow-doobleSmallShadow'>
              <Image src={UserPhoto} alt='User photo' className='absolute left-1/2 top-1/2 
              -translate-x-1/2 -translate-y-1/2 w-10 h-10' />

              <div role='presentation' 
                className='w-full h-full border-[3px] border-green rounded-full 
                transition duration-300 ease-in-out group-hover:border-[#0d7a50]'></div>
            </Button>
          </>
        )}
      </div>
